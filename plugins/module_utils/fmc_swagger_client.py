...
class FmcSwaggerParser:
    ...
    def _get_model_name_for_post_put_requests(self, params):
        ...
        if OperationField.PARAMETERS in params:
            ...
            x = []
            for i in y:
                if '$ref' not in i.keys():
                    x.append(i)
            body_param_dict = self._get_body_param_from_parameters(x)
    ...
    def _get_rest_params(self, params, global_param):
        ...
        for param in params:
            ...
            if '$ref' in param.keys():
                if param['$ref'] == '#/parameters/domainUUID':
                    param[PropName.NAME] = 'domainUUID'
                    path[param[PropName.NAME]] = self._simplify_param_def(global_param['domainUUID'])
                elif param['$ref'] == '#/parameters/expanded':
                    param[PropName.NAME] = 'expanded'
                    query[param[PropName.NAME]] = self._simplify_param_def(global_param['expanded'])
                elif param['$ref'] == '#/parameters/offset':
                    param[PropName.NAME] = 'offset'
                    query[param[PropName.NAME]] = self._simplify_param_def(global_param['offset'])
                elif param['$ref'] == '#/parameters/limit':
                    param[PropName.NAME] = 'limit'
                    query[param[PropName.NAME]] = self._simplify_param_def(global_param['limit'])
        return operation_param
...
class FmcSwaggerValidator:
    ...
    def _check_url_params(self, status, spec, params):
        ...
        # for prop_name in spec.keys():
        #     prop = spec[prop_name]
        #     if prop_name == 'domainUUID':
        #         if 'domainname' not in params:
        #             status[PropName.REQUIRED].append('domainname')
        #             continue
        #         if 'domainname' in params:
        #             expected_type = prop[PropName.TYPE]
        #             value = params['domainname']
        #             if not self._is_correct_simple_types(expected_type, value, allow_null=False):
        #                 self._add_invalid_type_report(status, '', 'domainname', expected_type, value)
        #             continue
        #     else:
        #         if prop[PropName.REQUIRED] and prop_name not in params:
        #             status[PropName.REQUIRED].append(prop_name)
    ...
    def _check_object(self, status, model, data, path):
        ...
        for prop in model_properties.keys():
            if prop in data:
                ...
                if PropName.TYPE in model_prop_val:
                    expected_type = model_prop_val[PropName.TYPE]
                elif PropName.REF in model_prop_val:
