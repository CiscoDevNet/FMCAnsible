...
* [`fmc_file_download.py`](library/fmc_file_download.py) - downloads files from FTD devices via HTTPS protocol;
* [`fmc_file_upload.py`](library/fmc_file_upload.py) - uploads files to FTD devices via HTTPS protocol;
* [`fmc_install.py`](library/fmc_install.py) - installs FTD images on hardware devices. The module performs a complete reimage of the Firepower system by downloading the new software image and installing it. 
...
2. You can build the custom Docker image:
    ```
    docker build -t fmc-ansible --build-arg PYTHON_VERSION=<2.7|3.5|3.6|3.7> --build-arg FTD_ANSIBLE_VERSION=<tag name | branch name> .
    ```
