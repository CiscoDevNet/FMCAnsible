...
DEFAULT_MODULE_PARAMS = dict(
    ...
    device_model=FmcModel.FMC_ASA5516_X.value,
    dns_server="8.8.8.8",
    console_ip="10.89.0.0",
    console_port="2004",
    console_username="console_user",
    console_password="console_pass",
    rommon_file_location="tftp://10.0.0.1/boot/fmc-boot-1.9.2.0.lfbff",
    image_file_location="http://10.0.0.1/Release/fmc-6.2.3-83.pkg",
...
class TestFmcInstall(object):
    ...
    def connection_mock(self, mocker):
        connection_class_mock = mocker.patch('ansible_collections.cisco.fmcansible.plugins.modules.fmc_install.Connection')
        return connection_class_mock.return_value

    @pytest.fixture
    def config_resource_mock(self, mocker):
        resource_class_mock = mocker.patch('ansible_collections.cisco.fmcansible.plugins.modules.fmc_install.BaseConfigurationResource')
        return resource_class_mock.return_value

    @pytest.fixture(autouse=True)
    def fmc_factory_mock(self, mocker):
        return mocker.patch('ansible_collections.cisco.fmcansible.plugins.modules.fmc_install.FmcPlatformFactory')

    @pytest.fixture(autouse=True)
    def has_kick_mock(self, mocker):
        return mocker.patch('ansible_collections.cisco.fmcansible.plugins.modules.fmc_install.HAS_KICK', True)

    def test_module_should_fail_when_kick_is_not_installed(self, mocker):
        mocker.patch('ansible_collections.cisco.fmcansible.plugins.modules.fmc_install.HAS_KICK', False)
    ...
    def test_module_should_return_when_software_is_already_installed(self, config_resource_mock):
        ...
        assert result['msg'] == 'FMC already has 6.3.0-11 version of software installed.'
    ...
    def test_module_should_install_fmc_image(self, config_resource_mock, fmc_factory_mock):
        ...
        fmc_factory_mock.create.assert_called_once_with('Cisco ASA5516-X Threat Defense', DEFAULT_MODULE_PARAMS)
    ...
    def test_module_should_fill_management_ip_values_when_missing(self, config_resource_mock, fmc_factory_mock):
        ...
        fmc_factory_mock.create.assert_called_once_with('Cisco ASA5516-X Threat Defense', expected_module_params)
