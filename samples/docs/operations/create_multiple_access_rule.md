...
| enabled | True | Boolean indicating whether the access rule is in effect (true) or not (false). Default is true. |
| type | AccessRule | Type of the response object. This value is always AccessRule. |
| name | Rule1 | User-specified name of the access rule. |
| sendEventsToFMC | False | Boolean indicating whether the device will send events to the Firepower Management Center event viewer. Default is false. If 'MONITOR' action is selected for access rule, sendEventsToFMC will always be true. |
| logFiles | False | Boolean indicating whether the device will log file events. Default is false. |
| logBegin | False | Boolean indicating whether the device will log events at the beginning of the connection. Default is false. If 'MONITOR' action is selected for access rule, logBegin will always be false. |
| logEnd | False | Boolean indicating whether the device will log events at the end of the connection. Default is false. If 'MONITOR' action is selected for access rule, logEnd will always be true. |
...
| section | False | string | This parameter specifies the section into which the rules will be added. If this parameter is not used the section will be the default section. Only 'mandatory' and 'default' are allowed values. If a section is specified, a category cannot be specified. |
| category | False | string | This parameter specifies the category into which the rules will be added. If a category is specified it must exist or the request will fail. If a section is specified, a category cannot be specified. |
